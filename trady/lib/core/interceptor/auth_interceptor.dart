...
class AuthInterceptor extends Interceptor {
  ...
  void onRequest(
    ...
    final listOfAiPaths = <String>[
     "${AppAPI.openAIBaseUrl}/v1/chat/completions"
    ];
    if (listOfPaths.contains(options.path.toString())) {
      return handler.next(options);
    }

    if (listOfAiPaths.contains(options.path.toString())) {
      final token2 = await cacheService.readCache(key: AppStrings.token2);

      options.headers.addAll({'Authorization': 'Bearer $token2'});
